...
def main():
    ...
    EAR_THRESHOLD = 0.20             # Below this, eyes are considered closed
    LONG_CLOSURE_SECONDS = 1.5       # Stopwatch limit for fatigue
    
    blink_count = 0                  # Total normal blinks
    is_currently_closed = False      # Boolean flag to know if we are in the middle of a blink
    eyes_closed_start_time = None    # When did the eyes first shut?

    # Capture the start time right before entering the loop (Industry-Grade)
    app_start_time = time.monotonic()

    with vision.FaceLandmarker.create_from_options(options) as landmarker:
        print("[INFO] Camera active. Blink to test counter. Close eyes for 1.5s to test fatigue warning.")
        try:
            while cap.isOpened():
                ...
                if detection_result.face_landmarks:
                    ...
                    if avg_ear < EAR_THRESHOLD:
                        # 1. Eyes just dropped below threshold (they just closed!)
                        if not is_currently_closed:
                            is_currently_closed = True
                            eyes_closed_start_time = current_time # Start the stopwatch!
                        else:
                            # 2. Eyes are STILL closed. Let's check how long it's been.
                            closure_duration = current_time - eyes_closed_start_time
                            if closure_duration >= LONG_CLOSURE_SECONDS:
                                cv2.putText(frame, "WARNING: FATIGUE / LONG CLOSURE!", (30, 130), 
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    else:
                        # 3. Eyes are open (above threshold)
                        if is_currently_closed:
                            # They were closed, but they just popped open!
                            closure_duration = current_time - eyes_closed_start_time
                            is_currently_closed = False # Reset our flag
                            
                            # Was it a quick blink or a long sleep?
                            if closure_duration < LONG_CLOSURE_SECONDS:
                                blink_count += 1 # It was short, count it as a blink!
